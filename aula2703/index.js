...
function withdraw(){
    inquirer.prompt([
        ...
        if(!checkAccount(accountName)) {
            console.error(`A conta: ${accountName} não existe, tente novamente!`)
            return withdraw()
        }
...
function removeAmount(accountName, amount){
    ...
    if(accountData.balance < amount){
       console.error(chalk.bgRed.white('O valor desejado é menor do que está em conta!'))
       return withdraw()
    }
    ...
    console.info(chalk.bgGreen.black(`O saque no valor de: ${amount} foi realizado com sucesso na conta ${accountName}`))
}